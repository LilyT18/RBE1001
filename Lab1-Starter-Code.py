...
def handleLeft1Button():
    ...
    if(current_state == IDLE):
        ...
        left_motor.spin_for(FORWARD, 5, TURNS, 60, RPM, wait = False)
        right_motor.spin_for(FORWARD, 5, TURNS, 60, RPM, wait = False)
...
def handleMotionComplete():
    ...
    if(current_state == DRIVING_FWD):
        ...
        current_state = DRIVING_BKWD
         ...
         ## TODO: You'll need to update the speed and number of turns       
        left_motor.spin_for(REVERSE, 5, TURNS, 60, RPM, wait = False)
        right_motor.spin_for(REVERSE, 5, TURNS, 60, RPM, wait = False)
